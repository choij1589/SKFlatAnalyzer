...
class MeasConversionV3(TriLeptonBase):
    def __init__(self):
        super().__init__()
    ...
    def defineObjects(self, rawMuons, rawElectrons, rawJets, syst="Central"):
        ...
        if not syst in self.systematics:
            raise ValueError(f"[PromptEstimator::defineObjects] Wrong systematic {syst}")
    ...
    def selectEvent(self, event, truth, vetoMuons, tightMuons, vetoElectrons, tightElectrons, jets, bjets, METv):
        ...
        if "DYJets" in super().MCSample and not region == "LowPT": return None
        if "ZGToLLG" in super().MCSample and not region == "HighPT": return None
